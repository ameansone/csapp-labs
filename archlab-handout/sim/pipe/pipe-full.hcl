...
quote   'int E_valP;'
word    e_valP = D_valP;
wordsig E_valP  'E_valP'
boolsig hook    'E_valP=gen_e_valP(),0'
...
word f_pc = [
	...
	M_icode == IJXX && M_ifun != 0 &&  M_Cnd && M_valA < M_valE &&
		M_valE != E_valP && !E_icode in {IJXX, ICALL, IRET} : M_valE;
...
bool D_bubble =
	...
	(E_icode == IJXX && E_ifun != 0 &&  e_Cnd && E_valA < E_valC &&
		E_valC != D_valP && !D_icode in {IJXX, ICALL, IRET} ) ||
	# Stalling at fetch while ret passes through pipeline
